...
[X_U, X_Sigma, X_V] = svd(X, 'econ'); %产生与X相同尺寸的对角线矩阵X_Sigma和具有递减顺序的非负对角线元素，以及矩阵X_U和X_V,使得X = X_U*X_Sigma*X_V'
X_rank = rank(X_Sigma); %RANK（A）提供矩阵A的线性独立行或列的数量的估计
X_U = X_U(:, 1:X_rank); 
...
if PrjX == 1    
    ...
    [W_x, eigenV_list_x] = solve_eigen(X_U, X_Sigma, X_V, H, RegX);   %W_x的每一列为一个特征向量，非零特征值对应特征向量 ，eigenV_list_x包含所有的特征值
