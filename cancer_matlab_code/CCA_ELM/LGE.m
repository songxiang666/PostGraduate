...
%                                          matrix
%                                           如果ReguType=Custom，则需要提供正则矩阵，nFea为数据矩阵的特征数
%
...
%                                   在PCA步骤中保留主成分的百分比，百分比是基于特征值计算的，默认是1，所有的非零特征值都会被保留。
%                                   如果>1，PCA将保持正确的主成分比例，不超过非零分量的精确数量。
%
%             Output:
